...
<div className="font-mulish grid lg:place-items-start place-items-center w-full bg-black text-purple-500 min-h-screen">
  <div className="flex lg:flex-row flex-col gap-5 lg:justify-start justify-center lg:items-start items-center w-full mx-auto">
    ...
    <h1 className="lg:text-5xl text-3xl py-8 lg:pt-14 uppercase font-medium text-purple-600">Solve It Out&apos;s</h1>
      ...
      {editTodo ? (
        ...
        <>
          <input
            className="w-full lg:w-8/12 bg-black border-2 border-purple-400 py-4 text-xl rounded-lg text-purple-400 outline-none px-3"
            type="text"
            value={newTodoText}
          ...
          />
          <button
            className="bg-slate-800 px-6 py-2 rounded-lg my-7 lg:my-12 text-green-400 text-lg uppercase font-semibold"
            onClick={addTodo}
          >
            Add Todo
          </button>
    ...
    <ul className="sm:w-9/12 lg:w-5/12 w-full px-4 flex flex-col justify-center items-center my-6 py-6 font">
      {  isLoading &&  <p className="text-pink-600 text-2xl italic my-10">Loading...</p>}
      {!isLoading && todos && todos.length == 0 ? (
        <div className="text-pink-600 text-2xl italic my-10">(  No todos in the list  )</div>
      ) : (
        <>
